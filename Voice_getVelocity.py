...
# for foo in pklDict['solenoid_time']: print(foo)

testIndex = 0 # Which strike to analyze 
...
if True:
    divCount = 40 # How many divisions to break audio up into
    divsPerSect = 3
    # divOverlap =- 1

    testLen = len(pklDict['signal'][testIndex]) # Length of each test
    divLen = m.floor(testLen / divCount) # Length of each subsection

    plotN = 5
    fig, ax = plt.subplots(1, plotN)
    freqCutoff = 2000


    for ii in range(plotN):
        fooPos = pklDict['stepper_position'][ii]
        fooVel = pklDict['solenoid_time'][ii]

        AmpSet = []
        for divIter in range(divCount - divsPerSect):
            div_start = divIter * divLen
            div_end = (divIter+divsPerSect) * divLen

            fooSig = pklDict['signal'][testIndex][div_start:div_end]

            FREQS_H, amps = af.FFT_Both(RATE, fooSig)
            # FREQS_H, amps = af.getHarmonic(RATE, fooSig)
            AmpSet.append(amps)



        maxInd = np.where(FREQS_H < freqCutoff)[0][-1]
        ax[testIndex%5].matshow(AmpSet, aspect='auto')
        # ax[testIndex%5].set_xticks([0])
        # ax[testIndex%5].set_xticks(FREQS_H[:maxInd][::m.floor(maxInd/15)][:-1])
        ax[testIndex%5].set_xticks(FREQS_H[::5])
        ax[testIndex%5].set_xlim(0, maxInd)

        estPitch, estConf = am.getFreqAndConf(pklDict['signal'][testIndex], RATE)
        ax[testIndex%5].set_title(str(round(estPitch, 1)) + ' : ' + str(fooVel))
...
if False:
    ...
    colSet = ['blue', 'orange', 'green', 'red', 'yellow']

    for sigIter in range(plotN):
        print(f"Loading  strike {sigIter}")

        times = []
        frq = []
        amp = []

        for subDivIter in range(minSubDiv, subDivCount+1):
            testLen = len(pklDict['signal'][testIndex]) # Length of each test
            divLen = m.floor(testLen / subDivIter) # Length of each subsection

            fooPos = pklDict['stepper_position'][testIndex]
            fooVel = pklDict['solenoid_time'][testIndex]

            # print(f"{subDivIter}:")
            for divIter in range(subDivIter):
                div_start = divIter * divLen
                div_end = (divIter+1) * divLen

                fooSig = pklDict['signal'][testIndex][div_start:div_end]

                # fooSig *= np.hanning(len(fooSig))
                # subsetFrqs, amps = af.FFT_Both(RATE, fooSig)
                subsetFrqs, amps = af.getHarmonic(RATE, fooSig, harmonicCount=1)

                frqPeaks, ampPeaks = af.FFT_Peaks(subsetFrqs, amps)

                for fooFrq, fooSig in zip(frqPeaks, ampPeaks/max(ampPeaks)):
                    # print(f"fooFrq:{fooFrq}      fooSig:{fooSig}")
                    times.append(div_start +divLen/2)
                    frq.append(fooFrq)
                    amp.append(fooSig)
        ...
        ax[sigIter%plotN].scatter(times, frq, alpha=0.1, color="blue")
        # ax[sigIter%plotN].scatter(times, frq, alpha=np.array(amp)*0.1, color="blue")
    ...
    #         # fooSig *= np.hanning(len(fooSig))
    #         # subsetFrqs, amps = af.FFT_Both(RATE, fooSig)
    #         subsetFrqs, amps = af.getHarmonic(RATE, fooSig, harmonicCount=2)
    ...
    # ax[sigIter%plotN].scatter(times, frq, alpha=0.1, color="green")
    # # ax[sigIter%plotN].scatter(times, frq, alpha=np.array(amp)*0.1, color="blue")
    ...
    for subDivIter in range(minSubDiv, subDivCount+1):
        ...
        for divIter in range(subDivIter):
            ...
            # fooSig *= np.hanning(len(fooSig))
            # subsetFrqs, amps = af.FFT_Both(RATE, fooSig)
            subsetFrqs, amps = af.getHarmonic(RATE, fooSig, harmonicCount=3)
    ...
    ax[sigIter%plotN].scatter(times, frq, alpha=0.1, color="orange")
    # ax[sigIter%plotN].scatter(times, frq, alpha=np.array(amp)*0.1, color="blue")
    ...
    # ax[sigIter%plotN].set_ylim(750, 1250)
    ax[sigIter%plotN].set_xticks([], [])
    # ax[sigIter%plotN].set_yticks([], [])
    ax[sigIter%plotN].set_title(str(fooVel))
    
    ax[sigIter%plotN].set_ylim(0, 1200)
    
    # ax[sigIter%plotN].plot(np.array(pklDict['signal'][testIndex]) + m.floor(sigIter/plotN)*10000)
    testIndex += 1


if False:
    divCount = 30 # How many divisions to break audio up into
    # divOverlap =- 1
    ...
    for ii in range(plotN):
        ...
        maxInd = np.where(FREQS_H < freqCutoff)[0][-1]
        ax[testIndex].matshow(AmpSet, aspect='auto')
        ax[testIndex].set_xticks(FREQS_H[:maxInd][::m.floor(maxInd/15)][:-1])
        ax[testIndex].set_xlim(0, maxInd)
        ax[testIndex].set_title(str(testIndex) + ' : ' + str(fooVel))

        testIndex += 1


# for ii in range(len(FREQS_H)):
#     print(f"{ii} : {FREQS_H[ii]}")
...
def loadPeaks(fooDict):
    ...
    while fooDict['stepper_position'][ii] <= fooDict['stepper_position'][ii+1]:
        ...
        if 'Right' in fooDict['solenoid_played'][ii]: fooPos *= -1
...
def getDataError(inPts, outPts, constSet):
    testPts = plotPow2Series(inPts, constSet)
    errorSet = abs(testPts - outPts)
...
best_error = getDataError(pos, frq, best_consts)
...
def updatePlots():
    ...
    ax.cla()
    ax.scatter(pos, frq, alpha=0.333, s=4)
...
while True:
    ...
    test_error = getDataError(pos, frq, test_consts)
...
ax.scatter(pos[goodInds], frq[goodInds], alpha=0.5)
ax.scatter(outDict['pos'], outDict['pitch'])
ax.scatter(pos[badInds], frq[badInds], alpha=0.5)
ax.set_ylim(min(frq)-25, max(frq)+25)
ax.set_ylabel("Note Frequency (Hz)")
ax.set_xlabel("Stepper Position (Steps)")
plt.draw()
