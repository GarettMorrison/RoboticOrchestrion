...
def getDataError(inPts, outPts, sigPts, constSet, allowableError):
...
def updatePlots(ax, pos, frq, best_consts, test_consts, testCount, sig):

    ax.cla()
    ax.scatter(pos, frq, alpha=sig*0.333, s=4)
    
    pltPts = np.arange(min(pos), max(pos), 1)
    ax.plot(pltPts, plotPow2Series(pltPts, best_consts), label = str(testCount))
...
def newConsts(best_consts, testCount, pos):
    halfTime = 500
    ...
    changeSel = m.floor(r.random()*11.0)
    match changeSel:
        case 0:
        ...
        case 6:
            modInd = m.floor(r.random()*(len(test_consts) -1) +1)
            test_consts[modInd] *= (modFactor*(4*r.random() - 2.0))
        case 7:
            for ii in range(2):
                modInd = m.floor(r.random()*(len(test_consts) -1) +1)
                test_consts[modInd] *= (modFactor*(4*r.random() - 2.0))
        ...
        case 8:
            rotatePos = (max(pos)-min(pos))*r.random()  +min(pos) # Get random point in range to change slope about
            preFrq = plotPow2Series(rotatePos, test_consts) # Get height of freq before modification
            test_consts[1] *= (2*modFactor*(r.random() - 0.5)) # Modify log slope
            postFrq = plotPow2Series(rotatePos, test_consts) # Get height of freq after modification
            test_consts[3] += preFrq - postFrq # Adjust height of point to match

        case 9:
            rotatePos = (max(pos)-min(pos))*r.random()  +min(pos) # Get random point in range to change slope about
            preFrq = plotPow2Series(rotatePos, test_consts) # Get height of freq before modification
            test_consts[2] *= (2*modFactor*(r.random() - 0.5)) # Modify slope
            postFrq = plotPow2Series(rotatePos, test_consts) # Get height of freq after modification
            test_consts[3] += preFrq - postFrq # Adjust height of point to match
        ...
        case 10:
            rotatePos = (max(pos)-min(pos))*r.random()  +min(pos) # Get random point in range to change slope about
            preFrq = plotPow2Series(rotatePos, test_consts) # Get height of freq before modification
            test_consts[4] *= (2*modFactor*(r.random() - 0.5)) # Modify lin slope
            postFrq = plotPow2Series(rotatePos, test_consts) # Get height of freq after modification
            test_consts[3] += preFrq - postFrq # Adjust height of point to match
...
def fitDataModel(pos, frq, sig, allowableError, ax):
    print("\n\n\nFITTING DATA\n")

    consts = [-1, -1, 1, 0, 0]
    # consts = [5000, 0.0005, 1, 1, 0.001]

    # Make first guess based on log2
    frqLog2 = np.log2(frq)
    fit_A, fit_B = np.polyfit(pos, frqLog2, 1)

    print(f"fit_A:{fit_A}")
    print(f"fit_B:{fit_B}")

    # Save slope of line as constants for log EQ
    consts[1] = fit_A
    consts[0] = fit_B/fit_A
    
    # pltPts = np.arange(min(pos), max(pos), 1)
    # ax.cla()
    # plt.plot(pltPts, plotPow2Series(pltPts, consts))

    # Subtract log correction to get linear slope
    frq_l2Err = frq - plotPow2Series(pos, consts)
    fit_A_2, fit_B_2 = np.polyfit(pos, frqLog2, 1)
    ...
    best_error = getDataError(pos, frq, sig, best_consts, allowableError)

    print(f"INIT ERROR   [", end='')
    for foo in best_consts: print("{:.4e}".format(foo), end=", ")
    print(f"]  err:{best_error}")
    ...
    plt.ion()
    changeSelCounts = []
    updatePlots(ax, pos, frq, best_consts, test_consts, testCount, sig)
    
    while True:
        print(f"{testCount}\033[1A")
        testCount += 1
        test_consts, changeSel = newConsts(best_consts, testCount, pos)
        test_error = getDataError(pos, frq, sig, test_consts, allowableError)
        ...
        if test_error < best_error:
            ...
            # Save changeSel
            while len(changeSelCounts) <= changeSel: changeSelCounts.append(0)
            changeSelCounts[changeSel] += 1
        
            updatePlots(ax, pos, frq, best_consts, test_consts, testCount, sig)

        # if testCount-lastImprovement > 500 or testCount > 2000: break
        if testCount > 40000: break
    
    for ii in range(len(changeSelCounts)): print(f"changeSel[{ii}] {changeSelCounts[ii]}")

    return(best_consts)



def getMatches(pos, frq, sig, frq_match, allowableError, ax):
    print("\n\n\nFINDING PITCH MATCHES to notes\n")
    ...
    if len(goodInds) < 1: 
        print("NO MATCHING PITCHES FOUND, EXITING")
        exit()
    ...
    return(outDict, goodInds, badInds)