...
{
private:
  uint32_t pulseDelay = 50000; // Delay between steps, in microseconds
  uint8_t drivePin;
  uint32_t currWait = 0;
...
dynStepper mainStep(22, 24, 26, 28); // Declare primary stepper class

solenoid driveSolenoid(52);
...
void loop() {
  ...
  if(Serial.available()){
    ...
    if(readInt == -1) driveSolenoid.pulse();
    else mainStep.setTarget(readInt);

    if(readInt > 0){Serial.write('>');}
    else if(readInt == -1){Serial.write('S');}
    else if(readInt < 0){Serial.write('<');}
    else{Serial.write('=');}
  }
